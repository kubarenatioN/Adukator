	...
	implements OnInit
{
	...
	public ngOnInit(): void {
		...
		this.personalization$ = combineLatest([
			...
			map(([topic, data]) => {
				return (
					data?.personalization.filter(
						(pers) => pers.task?.topicId === topic
					) ?? null
				);
			})
		);
		...
		this.checkTasks$ = this.topicCheckData$.pipe(
			...
			map(([checkData, viewData]) => {
				...
				const { topic: topicId, profile: profileId } =
					this.checkConfigForm.value;
				const topic = training.topics.find(
				...
				const tasksForCheck = [...tasks, ...personalTasks].map(
					(task) => {
	...
	private initResultsForm(
		training: StudentTraining,
		checkData: TopicCheckData
	) {
		...
		this.resultsFormMap = {};
		if (progress) {
			const formArray = this.getResultsFormArray(
				[...topicTasks, ...personalTasks],
				progress
			);
	...
	private getResultsFormArray(tasks: TopicTask[], profile: ProfileProgress) {
		const tasksLastResults = tasks.map((task) => {
			...
			const form = this.fbHelper.getTaskResultsCheckForm(
				lastResult ?? null
			);
	...
	private loadTopicCheckData(profileId: string, topicId: string) {
		return forkJoin({
			...
			personalization:
				this.personalizationService.getProfilePersonalization(
					profileId,
					'assignment'
				),
		});
	...
	private setupViewData(trainingId: string) {
		const training$ = this.getTraining(trainingId).pipe(filter(Boolean));
	...
	private getTraining(trainingId: string) {
		return this.teacherTraining
			.getCheckTraining(trainingId)
			.pipe(shareReplay(1));
	}

	private prepareTaskThreadData(
		task: TopicTask,
		profileId: string,
		discussion: TopicDiscussionReply[] | null
	) {
		...
		const thread = discussion
			.filter(
				(reply) => reply.type === 'task' && reply.taskId === task.id
			)
			.map((reply) => ({
				type: reply.type,
		...
		return {
			task,
			thread,
			folder: this.uploadService.getFilesFolder(
				'training',
				profileId,
				task.id
			),
