...
import { Course, CourseBundleCreatePayload } from 'src/app/typings/course.types';
...
export class CreateComponent implements OnInit {
  ...
  public addToBundle(course: Course) {
    this.teacherCourses = this.teacherCourses.filter(c => c._id !== course._id)
    this.bundleCourses.push(course)
    this.bundleForm.controls.courses.setValue([...this.bundleForm.value.courses ?? [], course])
  }

  public removeCourseFromBundle(course: Course) {
    this.bundleCourses = this.bundleCourses.filter(c => c._id !== course._id)
    ...
    this.teacherCourses = this.teacherCourses.slice().sort((a, b) => {
      return this.originalCoursesRef.indexOf(a) - this.originalCoursesRef.indexOf(b)
    })
  }
