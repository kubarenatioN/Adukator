...
export class ResponseTransformationInterceptor implements HttpInterceptor {
	...
	private transformTeacherCoursesData(body: {
		data?: TeacherCourses;
	}): Observable<any> {
		return this.configService.loadCourseCategories().pipe(
			map((categories) => {
				let { data } = body;
				if (data?.published) {
					data.published = this.processCoursesData(
						data.published,
						categories
					);
				}
				if (data?.review) {
					data.review = this.processCoursesData(
						data.review,
						categories
					);
				}
	...
	private getBodyTransformFn(
		reqType: string
	): (body: any) => Observable<any> {
		if (reqType === NetworkRequestKey.GetReviewCourseHistory) {
	...
	private processCoursesData(
		...
		data = data.map((course) => {
			course.categoryLabel = categories.find(
				(category) => category.key === course.category
			)?.name;
			return course;
		});
		return data;
